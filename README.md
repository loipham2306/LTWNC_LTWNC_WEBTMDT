# 🛒 Website Thương Mại Điện Tử - SÀN HIỆU

## 📌 Giới thiệu

Website thương mại điện tử bán hàng thời trang, hỗ trợ quản lý sản phẩm, đơn hàng, khách hàng, voucher, khuyến mãi và thống kê doanh thu.

---

# 📦 1. Quản lý Tài khoản (tai_khoan)

- [x] Đăng nhập
- [x] Đăng xuất
- [ ] Đổi mật khẩu
- [ ] Quên mật khẩu
- [X] Khóa/Mở khóa tài khoản
- [x] Phân quyền Admin/Nhân viên/Khách hàng
- [X] Xem danh sách tài khoản
- [X] Tìm kiếm tài khoản

---

# 👤 2. Quản lý Khách hàng (khach_hang)

- [ ] Thêm khách hàng
- [ ] Cập nhật thông tin khách hàng
- [ ] Xem chi tiết khách hàng
- [X] Khóa khách hàng
- [X] Danh sách khách hàng
- [X] Tìm kiếm khách hàng
- [ ] Thống kê khách hàng mới

---

# 👨‍💼 3. Quản lý Nhân viên (nhan_vien)

- [ ] Thêm nhân viên
- [ ] Sửa nhân viên
- [ ] Xóa nhân viên
- [ ] Danh sách nhân viên
- [ ] Tìm kiếm nhân viên
- [ ] Phân quyền nhân viên

---

# 📂 4. Quản lý Danh mục (danh_muc)

- [x] Thêm danh mục cha
- [x] Thêm danh mục con
- [x] Sửa danh mục
- [x] Xóa danh mục
- [x] Tìm kiếm danh mục
- [x] Lọc danh mục theo trạng thái

---

# 🏷️ 5. Quản lý Thương hiệu (thuong_hieu)

- [x] Thêm thương hiệu
- [x] Sửa thương hiệu
- [x] Xóa thương hiệu
- [x] Tìm kiếm thương hiệu
- [x] Xem mô tả thương hiệu
- [x] Ẩn/Hiện thương hiệu

---

# 👕 6. Quản lý Sản phẩm (san_pham)

- [x] Thêm sản phẩm
- [x] Sửa sản phẩm
- [x] Xóa sản phẩm
- [X] Chi tiết sản phẩm
- [X] Upload ảnh sản phẩm
- [x] Tìm kiếm sản phẩm
- [x] Lọc theo danh mục
- [x] Lọc theo thương hiệu
- [x] Lọc theo trạng thái
- [x] Phân trang sản phẩm

---

# 🎨 7. Quản lý Biến thể sản phẩm (bien_the_san_pham)

- [x] Thêm size
- [x] Thêm màu sắc
- [x] Quản lý tồn kho từng biến thể
- [x] Sửa biến thể
- [x] Xóa biến thể
- [x] Hiển thị danh sách biến thể

---

# 🛒 8. Giỏ hàng (gio_hang, chi_tiet_gio_hang)

- [x] Thêm vào giỏ hàng
- [x] Cập nhật số lượng
- [x] Xóa sản phẩm khỏi giỏ
- [x] Xem giỏ hàng
- [x] Tính tổng tiền
- [x] Áp dụng voucher
- [x] Chọn biến thể sản phẩm

---

# 📦 9. Đơn hàng (don_hang, chi_tiet_don_hang)

- [ ] Đặt hàng
- [ ] Hủy đơn hàng
- [ ] Xem lịch sử mua hàng
- [ ] Chi tiết đơn hàng
- [ ] Tìm kiếm đơn hàng
- [ ] Lọc theo trạng thái
- [ ] In hóa đơn

---

# 🚚 10. Lịch sử giao hàng (lich_su_giao_hang)

- [ ] Chờ xác nhận
- [ ] Đang chuẩn bị hàng
- [ ] Đang giao
- [ ] Đã giao
- [ ] Giao thất bại
- [ ] Theo dõi tiến trình đơn hàng

---

# 🎁 11. Voucher (voucher)

- [x] Thêm voucher
- [x] Sửa voucher
- [x] Xóa voucher
- [x] Kiểm tra hạn sử dụng
- [x] Kiểm tra số lần sử dụng
- [x] Áp dụng voucher khi thanh toán

---

# 🔥 12. Khuyến mãi (chuong_trinh_khuyen_mai)

- [ ] Tạo chương trình khuyến mãi
- [ ] Gắn sản phẩm vào khuyến mãi
- [ ] Giảm theo %
- [ ] Giảm theo số tiền
- [ ] Ngày bắt đầu
- [ ] Ngày kết thúc
- [ ] Tự động áp dụng giảm giá

---

# ⭐ 13. Bình luận & Đánh giá (binh_luan_va_danh_gia)

- [ ] Đánh giá sao (1-5 sao)
- [ ] Bình luận sản phẩm
- [ ] Sửa bình luận
- [ ] Xóa bình luận
- [ ] Duyệt bình luận
- [ ] Hiển thị đánh giá trung bình

---

# 📊 14. Thống kê Dashboard

- [ ] Tổng sản phẩm
- [ ] Tổng khách hàng
- [ ] Tổng đơn hàng
- [ ] Tổng doanh thu
- [ ] Đơn hàng hôm nay
- [ ] Sản phẩm bán chạy
- [ ] Danh mục bán chạy
- [ ] Thương hiệu bán chạy
- [ ] Biểu đồ doanh thu theo tháng

---

# 🌐 15. Chức năng Khách hàng

- [X] Đăng ký
- [x] Đăng nhập
- [x] Xem sản phẩm
- [x] Tìm kiếm sản phẩm
- [x] Lọc sản phẩm
- [x] Xem chi tiết sản phẩm
- [x] Thêm giỏ hàng
- [ ] Thanh toán
- [ ] Theo dõi đơn hàng
- [ ] Đánh giá sản phẩm
- [ ] Cập nhật hồ sơ cá nhân

---

## 🛠 Công nghệ sử dụng

- PHP MVC
- MySQL
- Bootstrap 5
- JavaScript
- HTML5/CSS3

Host: fdb1032.awardspace.net
Database: 4767098_tramhieu
Username:4767098_tramhieu
Password: Vn@Shop#2026
## 👨‍💻 Thành viên thực hiện

- Nguyễn Minh Luân
- Phạm Hữu Lợi